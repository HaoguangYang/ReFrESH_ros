...
void ReFRESH_Module::halt() {
  ...
  // Stop EV
  if (childrenCount() > 1) ControlNode::haltChild(1);
  // Stop ES
  if (childrenCount() > 2) ControlNode::haltChild(2);
  // initialEV_ = false;
  asyncEV_ = false;
  prestartES_ = false;
...
BT::NodeStatus ReFRESH_Module::tick() {
  ...
  switch (EXstatus) {
    case NodeStatus::SUCCESS:
      ...
      // evaluate_() == SUCCESS (successfully completed, we need to check consistency just to be
      // safe)
      if (pCost_ >= 1.0) {
        return NodeStatus::FAILURE;
      }
